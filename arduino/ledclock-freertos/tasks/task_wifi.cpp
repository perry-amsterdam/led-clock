...
static void push_led(uint8_t r,uint8_t g,uint8_t b,uint16_t ms){ LedCmd c{r,g,b,ms}; xQueueSend(g_ledQueue,&c,0); }
...
void task_wifi(void*){
  ...
  for(;;){
    if(!rtos_test_bits(EVT_WIFI_UP) && !rtos_test_bits(EVT_PORTAL_ON)){
      ...
      while(hal_millis()-t0 < 10000 && WiFi.status()!=WL_CONNECTED){ hal_delay_ms(50); }
