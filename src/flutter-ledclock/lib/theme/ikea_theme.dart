...
MaterialStateProperty<Color?> overlayFrom(Color base, {double hover = .08, double pressed = .14, double focus = .12}) {
  return MaterialStateProperty.resolveWith((states) {
    if (states.contains(MaterialState.pressed)) return base.withOpacity(pressed);
    if (states.contains(MaterialState.hovered)) return base.withOpacity(hover);
    if (states.contains(MaterialState.focused)) return base.withOpacity(focus);
    return null;
  });
}

ButtonStyle largeButtonStyle(ColorScheme scheme) {
  final shape = RoundedRectangleBorder(borderRadius: BorderRadius.circular(IkeaTokens.radius));
  return ButtonStyle(
